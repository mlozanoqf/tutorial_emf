---
title: "**Forecast with automatic machine learning and other techniques.**"
author:
- Dr. Martín Lozano.
...
toc: yes
abstract: We tackle a single forecast problem using several techniques. This document is based on some freely available Business Science IO codes in R that explains how to implement machine learning workflow using H2O. I added some additional explanations and code for teaching purposes (literate programming). This is still a work in progress and it is under revision.
---
...
Let's have a look of the data set. By default it says \texttt{price}, but these are basically sales figures in monetary terms. According to the main FRED reference, these are in millions of dollars, not seasonally adjusted.
...
The main objective here is to use \texttt{h2o} locally (in your own computer) to develop a high accuracy time series model on the \texttt{beer\_sales\_tbl} data set. This is a supervised machine learning regression problem. An interesting reference to learn the basics of supervised and unsupervised machine learning techniques applied to business is: Machine Learning in Business: An Introduction to the World of Data Science (2019), by John C. Hull. 

## Prepare the data.

The \texttt{tk\_augment\_timeseries\_signature} function expands out the timestamp information column-wise into a machine learning feature set, adding columns of time series information to the original data frame. We’ll again use \texttt{glimpse} for quick inspection of this expansion. See how there are now 31 features extracted from the original database. Not all will be important for the final and chosen models, but some will.
...
Note how we went from 3 columns in \texttt{beer\_sales\_tbl} to 31 columns in \texttt{beer\_sales\_tbl\_aug}, almost *out of the blue*.

We need to prepare the data in a format for H2O. First, let’s remove any unnecessary columns such as dates or those with missing values, and change the ordered classes to plain factors. We prefer \texttt{dplyr} operations for these steps. Sometimes we do not need to implement this step as the data is already clean (as in this case), but sometimes it is not. Thus, let's clean the data.
...
The database did not change too much. Now we have 29 columns in \texttt{beer\_sales\_tbl\_clean}. In the case of two variables, the structure ordered factors \texttt{<ord>} changed into factors \texttt{<fct>}, which is necessary for some H2O functions.
...
The \texttt{h2o.automl} is a function in H2O that automates the process of building a large number of models, with the goal of finding the *best* model without any prior knowledge or effort by the data scientist. The alternative of using \texttt{h2o.automl} is to pick some models according to the database characteristics, implement the models, and pick the one with the best performance according to some evaluation criterion. This alternative is time consuming and it could use an intensive computational memory and power, this is why H2O is valuable. If H2O was already amazing, this function makes it even more powerful. 

The available algorithms that \texttt{h2o.automl} currently run and compare are (click on each one to see a full description):
...
It is a good time to define how we are going to use some concepts at least in this document. Here, we call forecasting *techniques* to the three techniques implemented in this document: machine learning using H2O, linear regression, and ARIMA. When we implement \texttt{h2o.automl} function, H2O test for the six *algorithms* listed above. Each algorithm includes many other *models* that belongs to these algorithms in the machine learning process. The result of \texttt{h2o.automl} is one model that belongs to one algorithm. This is the difference between forecasting techniques, algorithms, and models. 

## Implement \texttt{h2o.automl}.

Here, we implement the \texttt{h2o.automl} in three different ways because of reproducibility issues. Reproducibility means obtaining consistent computational results using the same input data, computational steps, methods, code, and conditions of analysis. It turns out that Deep Learning cannot be reproducible by construction. Then, we first apply \texttt{h2o.automl} without Deep Learning. Second, we apply \texttt{h2o.automl} with only Deep Learning (here the results will be different each time we run the code). And third, including all available algorithms in \texttt{h2o.automl} (again, the results might change every time we run the code). The first is the only one which can be reproducible and the other two are expected to change every time we run the R code. 

Please note that in the code below we set \texttt{exclude\_algos} to exclude Deep Learning, and \texttt{seed = 236} to make sure every time we run the code we can get the same results.
...
The selected model by \texttt{h2o.automl} is:
...
The \texttt{model\_id} column list the top 10 models with the lowest errors. The value of \texttt{h2o.automl} is that we can take the best model and use it to conduct our forecast. Remember we proposed to run \texttt{h2o.automl} three times. Now let's consider the second alternative (only Deep Learning). There are several ways to implement Deep Learning, this is why it makes sense to use only this family into the \texttt{h2o.automl} function. Deep Learning cannot be reproducible by construction so adding a seed in this case would be useless. 
...
Let's summarize the results according to the mean residual deviance as this was the criterion in \texttt{stopping\_metric}. The table shows the best ranked model according to our three different runs of \texttt{h2o.automl}.
...
This is interesting because this suggest that it makes sense to run the H2O more than one time. It would be good to test for a different \texttt{stopping\_metric}, \texttt{max\_runtime\_secs} and \texttt{max\_models}.
...
We can also show the best point forecast for the three \texttt{h2o.automl} runs.
...
Let's implement a simple but powerful approach using the \texttt{lm} function.

## Implement the model.

This is the simplest choice, and still has a very high $R^2$. The independent variables are all \texttt{beer\_sales\_tbl\_aug} variables except for \texttt{date}, \texttt{diff}, and \texttt{symbol}.
...
Here, \texttt{sweep} is used for tidying the \texttt{forecast} package workflow. We’ll work through an ARIMA analysis to forecast the next 10 months of time series data. In this way we can compare our previous results.

## Prepare the data.

The \texttt{tk\_ts} coerce time series objects and tibbles with date/date-time columns to ts (time-series).
...
Just verify \texttt{tk\_ts} worked.
...
Great. This will be important when we use \texttt{sw\_sweep} later. Next, we’ll model using ARIMA.

## Implement the model.

We can use the \texttt{auto.arima} function from the forecast package to model the time series. By doing that, we do not have to impose a specific ARIMA model, the function can test the best specification for us.
...
The \texttt{sw\_tidy} function returns the model coefficients in a tibble (tidy data frame). This might be useful in some circumstances.
...
The \texttt{sw\_glance} function returns the training set accuracy measures in a tibble (tidy data frame). We use glimpse to aid in quickly reviewing the model metrics.
...
The \texttt{sw\_augument} function helps with model evaluation. We get the “.actual”, “.fitted” and “.resid” columns, which are useful in evaluating the model against the training data. Note that we can pass \texttt{timetk\_idx = TRUE} to return the original date index.
...
Make a forecast using the \texttt{forecast} function. This function also delivers some convenient error bounds.
...
We can use \texttt{sw\_sweep} to tidy the forecast output. As an added benefit, if the forecast-object has a \texttt{timetk} index, we can use it to return a date/datetime index as opposed to regular index from the ts-based-object.
...
Great. Now, use \texttt{sw\_sweep} to tidy the forecast output.
