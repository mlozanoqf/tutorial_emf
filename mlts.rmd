...
beer_tbls %>%
  ...
  components() %>%
  autoplot() +
  labs(title = "A decomposition of beer sales obtained using SEATS.")
```
...
beer_tbls %>%
  ...
  labs(y = "Sales", x = "Date",
       title = "Seasonal plot: Beer sales.")
```
...
beer_tbls %>%
  gg_subseries(sales) +
  labs(y = "Sales",
    title = "Seasonal subseries plot of monthly beer sales.")
```

```{r}
beer_tbls %>%
  ACF(sales, lag_max = 48) %>%
  autoplot() + labs(title = "ACF of monthly beer sales.")
```
...
```{r exclude-deeplearning takes a few minutes to run, include=FALSE}
...
After `r as.numeric((t_inner1$toc-t_inner1$tic)/60)` minutes running, the selected model by <tt>`h2o.automl()`</tt> is:
...
t_inner2 <- toc()
```
After `r as.numeric((t_inner2$toc-t_inner2$tic)/60)` minutes running, the selected model by <tt>`h2o.automl()`</tt> is:
...
After `r as.numeric((t_inner3$toc-t_inner3$tic)/60)` minutes running, the selected model by <tt>`h2o.automl()`</tt> is:
...
geom_point(size = 5, alpha = 0.5) +
    ...
    geom_point(aes(x = date, y = sales), color = "black", size = 5,
        ...
        geom_vline(xintercept = as.numeric(as.Date("2021-12-01")), 
                  linetype = 2) +
    # Aesthetics
labs(title = "Beer Sales Forecast: ARIMA", x = "", y = "Sales",
