...
```{r fig.cap="Zoom: Beer, Wine, and Distilled Alcoholic Beverages Sales."}
beer %>%
...
```{r fig.cap="A multiplicative c of beer sales using X-11."}
beer_tbls %>%
...
beer_tbls %>%
  ...
  head(8)
```
The consumption trend significantly increased from June 2020 to July 2020. There are also three negative spikes in the irregular component.
...
```{r fig.cap="A decomposition of beer sales obtained using SEATS."}
beer_tbls %>%
...
```{r fig.cap="Seasonal plot: Beer sales."}
beer_tbls %>%
...
```{r fig.cap="Zoom. Seasonal plot: Beer sales."}
beer_tbls %>%
...
```{r fig.cap="Seasonal subseries plot of monthly beer sales."}
beer_tbls %>%
  gg_subseries(sales) +
  labs(y = "Sales", x = "Date")
```
December is the month of the year with the highest sales, followed by June. January is the month of the year with the lowest sales, followed by February.
...
```{r fig.cap="ACF of monthly beer sales."}
beer_tbls %>%
...
Let's estimate four simple forecast techniques. These are: mean (the forecasts of all future values are equal to the average of the historical data), naïve (set all forecasts to be the value of the last observation), seasonal naïve (set each forecast to be equal to the last observed value from the same season), and drift (to allow the forecasts to increase or decrease over time).
...
```{r fig.cap="Four simple forecasts."}
beer_fc %>%
...
```{r fig.cap="Zoom: Four simple forecasts."}
beer_zoom <- beer_tbls %>%
  select(date, sales) %>%
...
```{r fig.cap="Zoom: Seasonal naïve forecasts."}
beer_zoom <- beer_tbls %>%
  select(date, sales) %>%
  filter_index("2019-12" ~ .)
...
beer_sn %>%
  autoplot(beer_zoom, lwd = 2, alpha = 0.4)  +
  geom_vline(xintercept = as.Date("2022-01-01"), lty = 2) +
  labs(y = "Sales", x = "Date") #+
  #guides(colour = guide_legend(title = "Forecast"))
```
The forecast looks accurate. Let's compute the MAPE of all forecasts.
...
fabletools::accuracy(beer_fc, beer_2022) %>%
  select(.model, MAPE)
```
...
```{r fig.cap="ARIMA fpp3 forecasts."}
beer_arima_fc %>%
  autoplot(beer_tbls, level = NULL) +
  geom_vline(xintercept = as.Date("2022-01-01"), lty = 2) +
  labs(y = "Sales", x = "Date") +
...
```{r}
fabletools::accuracy(beer_arima_fc, beer_2022) %>%
  select(.model, MAPE)
```
...
See why `r automl_leader@algorithm` was the chosen one:
...
After `r round(as.numeric((t_inner2$toc-t_inner2$tic)/60), 2)` minutes running, the selected model by <tt>`h2o.automl()`</tt> is:
...
See why this specific `r automl_DL@algorithm` model was the chosen one:
