...
```{r fig.cap="fpp3: four simple forecasts."}
beer_fc %>%
...
```{r fig.cap="Zoom. fpp3: four simple forecasts."}
beer_zoom <- beer_tbls %>%
  select(date, sales) %>%
...
```{r fig.cap="Zoom. fpp3: seasonal naïve forecasts."}
beer_zoom <- beer_tbls %>%
  select(date, sales) %>%
  filter_index("2019-12" ~ .)
...
```{r fig.cap="fpp3: ETS(M,A,M) forecast."}
beer_ets %>%
  fabletools::forecast(h = 10) %>%
  autoplot(beer_tbls) +
  geom_vline(xintercept = as.Date("2022-01-01"), lty = 2) +
...
```{r fig.cap="Zoom. fpp3: ETS(M,A,M) forecast."}
  ggplot(beer_zoom, aes(yearmonth(date), sales), lwd = 2, alpha = 0.4) +
  geom_line() +
  geom_point(size = 5, color = "black", alpha = 0.5, 
             shape = 21, fill = "black") +
...
```{r fig.cap="Residuals from the fitted fpp3 ARIMA(4,1,1)(0,1,1)[12] model."}
beer_arima %>%
  select(arima_auto) %>%
  gg_tsresiduals()
```
...
```{r fig.cap="fpp3: ARIMA(4,1,1)(0,1,1)[12] forecast."}
beer_arima_fc %>%
  autoplot(beer_tbls, level = NULL) +
  geom_vline(xintercept = as.Date("2022-01-01"), lty = 2) +
  labs(y = "Sales", x = "Date") +
...
```{r fig.cap="Zoom. fpp3: ARIMA(4,1,1)(0,1,1)[12] forecast."}
  ggplot(beer_zoom, aes(yearmonth(date), sales), lwd = 2, alpha = 0.4) +
  geom_line() +
  geom_point(size = 5, color = "black", alpha = 0.5, 
             shape = 21, fill = "black") +
...
```{r fig.cap="fpp3: NNAR(15,1,8)[12] forecast."}
beer_nnet_fc %>%
  autoplot(beer_tbls, level = NULL) +
  geom_vline(xintercept = as.Date("2022-01-01"), lty = 2) +
  labs(y = "Sales", x = "Date") +
...
```{r fig.cap="Zoom. fpp3: NNAR(15,1,8)[12] forecast."}
  ggplot(beer_zoom, aes(yearmonth(date), sales), lwd = 2, alpha = 0.4) +
  geom_line() +
  geom_point(size = 5, color = "black", alpha = 0.5, 
             shape = 21, fill = "black") +
