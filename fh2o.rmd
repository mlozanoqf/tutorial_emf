...
The variable (column) names <tt>`beer_aug`</tt> are:
...
When we implement <tt>`h2o.automl()`</tt> function, H2O test for the six *algorithms* listed above. Each algorithm includes many other *models* that belongs to these algorithms in the machine learning process. The result of <tt>`h2o.automl()`</tt> is one model that belongs to one algorithm. This is the difference between forecasting techniques, algorithms, and models. 
...
Please note that in the code below we set <tt>`exclude_algos`</tt> to exclude Deep Learning, and <tt>`seed = 13`</tt> to make sure every time we run the code we can get the same results.

```{r exclude-deeplearning takes a few minutes to run}
...
After `r round(as.numeric((t_inner1$toc-t_inner1$tic)/60), 2)` minutes running, the selected model by <tt>`h2o.automl()`</tt> is:
...
See why `r automl_leader@algorithm` was the chosen one. Model rankings: <tt>`h2o.automl()`</tt> without Deep Learning algorithm.

```{r}
# Model rankings: h2o.automl without Deep Learning algorithm.
head(automl_models_h2o@leaderboard[,1:2], 10)
```
...
After `r round(as.numeric((t_inner2$toc-t_inner2$tic)/60), 2)` minutes running, the selected Deep Learning model by <tt>`h2o.automl()`</tt> is:
...
See why this specific `r automl_DL@algorithm` model was the chosen one. Model rankings: <tt>`h2o.automl()`</tt> with only Deep Learning algorithm.

```{r}
head(DL@leaderboard[,1:2], 10)
```
...
After `r round(as.numeric((t_inner3$toc-t_inner3$tic)/60), 2)` minutes running, the selected model by <tt>`h2o.automl()`</tt> is:
...
See why c model was the chosen one in this specific and unique code compilation. Model rankings: <tt>`h2o.automl`</tt> with all available algorithms

```{r}
head(automl_models_h2o_all@leaderboard[,1:2], 10)
```

Let's summarize the results according to the root mean square error *rmse* as this was the criterion in <tt>`stopping_metric`</tt>. The table shows the best ranked model according to our three different runs of <tt>`h2o.automl()`</tt>.
...
```{r}
#| label: fig-fhwdla
#| fig-cap: "Forecast. H2O without Deep Learning algorithm."
beer %>%
...
```{r}
#| label: fig-fhiodla
#| fig-cap: "Forecast. H2O including only Deep Learning algorithm."
beer %>%
    filter(date > as.Date("2021-01-01")) %>%
    ggplot(aes(x = yearmonth(date), y = sales)) +
    geom_point(size = 5, color = "black", alpha = 0.5, 
...
Finally, this is the H2O case with no restrictions, considering all available algorithms.
...
```{r}
#| label: fig-fhiaaa
#| fig-cap: "Forecast. H2O including all available algorithms."
beer %>%
    filter(date > as.Date("2021-01-01")) %>%
    ggplot(aes(x = yearmonth(date), y = sales)) +
    geom_point(size = 5, color = "black", alpha = 0.5, 
...
In finance we care about the future and these techniques can be used as a tool to reduce the uncertainty about the future. Obviously, we cannot predict without errors, but the objective is to achieve the lowest forecasting errors possible. 
...
# Two more forecast models.

As in the original Matt Dancho's example, we include the linear regression and a different ARIMA package to conduct two more forecasts using the <tt>`auto.arima()`</tt> function.

## Linear regression.

Let's implement a simple approach using the <tt>`lm()`</tt> function.

## Estimation.

This is the simplest choice, and still has a very high adjusted $R^2$. The independent variables are all <tt>`beer_aug`</tt> variables except for <tt>`date`</tt>, <tt>`diff`</tt>, and <tt>`symbol`</tt>.
...
```{r}
#| label: fig-fmlr
#| fig-cap: "Forecast. Multivariate linear regression."
beer %>%
  filter(date > as.Date("2021-01-01")) %>%
  ggplot(aes(x = yearmonth(date), y = sales)) +
  geom_line(color = "black", size = 0.5) +
...
This is clearly a good alternative.
...
The <tt>`tk_ts()`</tt> function coerce time series objects and tibbles with date/date-time columns to <tt>`ts`</tt> (time-series).
...
Just verify <tt>`tk_ts()`</tt> function worked.
...
Great. This will be important when we use <tt>`sw_sweep`</tt>. Next, we’ll model using ARIMA.

## Estimation.

We can use the <tt>`auto.arima()`</tt> function from the <tt>`forecast`</tt> package to model the time series. By doing that, we do not have to impose a specific ARIMA model, the function can test the best specification for us.
...
The results are somewhat different to the ARIMA <tt>`fpp3`</tt> alternative. The <tt>`sw_tidy()`</tt> function returns the model coefficients in a tibble (tidy data frame). This might be useful in some circumstances.
...
The <tt>`sw_glance()`</tt> function returns the training set accuracy measures in a <tt>`tibble`</tt>. We use `</tt>glimpse()`</tt> function to aid in quickly reviewing the model metrics.
...
```{r}
#| label: fig-fard
#| fig-cap: "Forecast. ARIMA(1,1,2)(2,1,1)[12] residual diagnosis."
sw_augment(fit_arima, timetk_idx = TRUE) %>%
...
Notice that we have the entire forecast in a <tt>`tibble`</tt>. We can now more easily visualize the forecast.

```{r}
#| label: fig-fa
#| fig-cap: "Forecast. ARIMA(1,1,2)(2,1,1)[12]."
fcast_tbl %>%
...
We need to add our H2O forecast into <tt>`fpp3_fc`</tt>.
...
Now gather all together, calculate the average forecast, and performance measures.
