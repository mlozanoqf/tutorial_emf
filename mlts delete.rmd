...
```{r fig.cap="A multiplicative decomposition of beer sales using X-11."}
beer_tbls %>%
...
```{r fig.cap="Three negative spikes in the irregular component."}
beer_tbls %>%
  model(x11 = X_13ARIMA_SEATS(sales ~ x11())) %>%
  components() %>%
...
```{r fig.cap="Residuals from the fitted ARIMA(4,1,1)(0,1,1)[12] model."}
beer_arima %>%
  select(arima_auto) %>%
  gg_tsresiduals()
```
...
```{r fig.cap="ARIMA(4,1,1)(0,1,1)[12] forecast."}
beer_arima_fc %>%
  autoplot(beer_tbls, level = NULL) +
  geom_vline(xintercept = as.Date("2022-01-01"), lty = 2) +
  labs(y = "Sales", x = "Date") 
...
```{r fig.cap="Zoom. ARIMA(4,1,1)(0,1,1)[12] forecast."}
  ggplot(beer_zoom, aes(date, sales), lwd = 2, alpha = 0.4) +
  geom_line() +
  geom_point(size = 5, color = "black", alpha = 0.5, 
             shape = 21, fill = "black") +
...
report(beer_nnet)
```
The $NNAR(15,1,8)_{12}$ model has inputs $y_{t−1}, y_{t−2},..., y_{t−15}$ and 8 neurons in the hidden layer. 
...
```{r fig.cap="NNAR(15,1,8)[12] forecast."}
beer_nnet_fc %>%
  autoplot(beer_tbls, level = NULL) +
  geom_vline(xintercept = as.Date("2022-01-01"), lty = 2) +
  labs(y = "Sales", x = "Date") 
...
```{r fig.cap="Zoom. NNAR(15,1,8)[12] forecast."}
  ggplot(beer_zoom, aes(date, sales), lwd = 2, alpha = 0.4) +
  geom_line() +
  geom_point(size = 5, color = "black", alpha = 0.5, 
             shape = 21, fill = "black") +
